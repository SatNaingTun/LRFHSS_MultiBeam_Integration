...
def atmospheric_gases_plot():
    ...
    :return:
    """
    f = ProjectConfig.CENTER_FREQUENCY_HZ * 1e-9 * u.GHz
    p = ProjectConfig.p
    D = ProjectConfig.D * u.m
    lat_GS = ProjectConfig.LATITUDE_CENTER_DEG
    lon_GS = ProjectConfig.LONGITUDE_CENTER_DEG
...
def get_array_steering_vector(satellite_pos, user_pos):
    ...
    """
    n_beams = ProjectConfig.N_BEAMS_X * ProjectConfig.N_BEAMS_Y
    n_antenna_x = ProjectConfig.N_ANTENNA_X
    n_antenna_y = ProjectConfig.N_ANTENNA_Y
...
def get_Rician_fading_coefficient(n_user):
    ...
    """
    rician_k = ProjectConfig.rician_k
...
def get_satellite_Doppler_shift():
    ...
    """
    v_sat = ProjectConfig.V_SATELLITE
    SPEED_OF_LIGHT = ProjectConfig.SPEED_OF_LIGHT
    center_freq = ProjectConfig.CENTER_FREQUENCY_HZ
...
def get_effective_channel(loss_db, precoder_analog, i_sat_pos, user_pos, n_user, i_frame):
    ...
    antenna_gain = ProjectConfig.GAIN_RX
    tx_power_W = 10 ** (ProjectConfig.TX_PWR_DB / 10)
